...
cc.Class({
    ...
    prepareReplay: function (roomInfo, detailOfGame) {
        ...
        if( detailOfGame.base_info.jin)
        {
            this.jin = detailOfGame.base_info.jin;
    ...
    getWanfa: function () {
        var conf = this.conf;
        //console.log("liuhai"+conf.maxGames);
        if (conf) {
            if(!conf.maxGames){
                var strArr = [];
                strArr.push("回放中");
            ...
            if (conf.maxGames != null && conf.kunjin != null && conf.jinfengding != null && conf.baseScore != null && conf.jinfen != null) {
                var strArr = [];
                strArr.push(conf.maxGames + "局");
                strArr.push(conf.jinfengding + "金封顶");
                if (conf.kunjin) {
                    strArr.push("捆金");
                } else {
                    strArr.push("不捆金");
    ...
    ongetLocation(latitude) {


        //  cc.vv.http.sendRequest("/get_position", data);
        cc.vv.userMgr.latitude = latitude;
       
        var data = {
            account: cc.vv.userMgr.account,
            sign: cc.vv.userMgr.sign,
            location: cc.vv.userMgr.latitude,

        }
        cc.vv.http.sendRequest("/set_location", data);
        console.log(data);
    ...
    initHandlers: function () {
        ...
        cc.vv.net.addHandler("game_PKholds_push",function(data){

            var seat = self.seats[self.seatIndex];
            console.log(data);
            seat.holds = data;
            self.dispatchEvent('game_PKholds');
        ...
        cc.vv.net.addHandler("game_sync_push", function (data) {
            ...
            self.jin = [];
            if(data.isQiHu){
                for (var i = 0; i < data.isQiHu.length; i++) {
                    self.isQiHu.push(data.isQiHu[i]);
                }
            ...
            if(data.isTing){
                ...
                for (var i = 0; i < data.isTing.length; i++) {
                    self.isTing.push(data.isTing[i]);
                }
        ...
        cc.vv.net.addHandler("game_getjin_push", function (data) {
            self.gamestate = "chujin";
            self.jin = [];
            for (var i = 0; i < data.length; i++) {
                self.jin.push(data[i]);
            }
            console.log(self.jin);
            console.log(data);
            self.dispatchEvent("game_getjin", data);
        });
        ...
        cc.vv.net.addHandler("guo_hu_notify_push", function (data) {
            var userId = data;
            var si = self.getSeatIndexByID(userId);
            var localsi = self.getLocalIndex(si);
            self.dispatchEvent('guohu_notify', localsi);
        ...
        cc.vv.net.addHandler("game_ting_notify_push", function (data) {
            var userId = data.userId;
            var si = self.getSeatIndexByID(userId);
            var pai = data.pai;
            var localsi = self.getLocalIndex(si);
            self.dispatchEvent('ting', localsi);
            console.log("game_ting_notify_push"+data);
            cc.vv.audioMgr.playSFX("tingpai.mp3");
            if (data.isBaoTing) {
                self.doTingpai(si, pai);
            }
        ...
        cc.vv.net.addHandler("game_ready_finish_push", function (data) {
            // for(var i = 0; i < data.length; ++i){
            ...
            //     }
            // }
            self.dispatchEvent('game_ready_finish', data);
        });
    ...
    doTingpai: function (seatIndex, pai) {
        this.chupai = pai;
        var seatData = this.seats[seatIndex];
        if (seatData.holds) {
            var idx = seatData.holds.indexOf(pai);
