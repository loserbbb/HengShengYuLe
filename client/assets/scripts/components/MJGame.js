cc.Class({
    ...
    onLoad: function () {
       ...
       // cc.vv.net.send("get_game_getjin");
    },
    ...
    initView:function(){
        ...
        gameChild.getChildByName("jininfo").active = false;
        if(cc.vv.gameNetMgr.conf.type == 'sjmj' || cc.vv.gameNetMgr.conf.type == 'djmj' || cc.vv.gameNetMgr.conf.type == 'kdd'){
            gameChild.getChildByName("jininfo").active = true;
            if(cc.vv.gameNetMgr.conf.type == 'kdd'){
                gameChild.getChildByName("jininfo").getComponent(cc.Label).string = "风\n耗\n子"
            }else{
                gameChild.getChildByName("jininfo").getComponent(cc.Label).string = "金\n牌"
            }
        }
        gameChild.getChildByName("jininfo").getChildByName("jp1").active = false;
        gameChild.getChildByName("jininfo").getChildByName("jp2").active = false;
        this._jin = [];

        for(i = 0;i < cc.vv.gameNetMgr.jin.length; i++){
            this._jin.push(cc.vv.gameNetMgr.jin[i]);
            var jp = "jp" + (i+1);
            var jinSprite = gameChild.getChildByName("jininfo").getChildByName(jp).getComponent(cc.Sprite);
            gameChild.getChildByName("jininfo").getChildByName(jp).active = true;
            jinSprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",this._jin[i]);

        }
        if(!cc.vv.gameNetMgr.conf.fenghaozi&&cc.vv.gameNetMgr.conf.type=="kdd"){
            this.node.getChildByName("game").getChildByName("jininfo").active = false;
        }


        
        // var jinSprite1 = gameChild.getChildByName("jininfo").getChildByName("jp1").getComponent(cc.Sprite);
        // var jinSprite2 = gameChild.getChildByName("jininfo").getChildByName("jp2").getComponent(cc.Sprite);
        // jinSprite1.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",this._jin[0]);
        // jinSprite2.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",this._jin[1]);
        ...
        for(var i = 0; i < sides.length; ++i){
            ...
            this._tingpaiTips.push(sideChild.getChildByName("TingPai"));
            this._qihus.push(sideChild.getChildByName("qihu"));
    ...
    initEventHandlers:function(){
        ...
        this.node.on('game_PKholds',function(data){
            self.initPokes();
        })
        ...
        this.node.on('',function(data){
            self.onGameBeign();
            self.checkIp();
        });
        this.node.on('game_getjin',function(data){
            self._jin = [];
            data = data.detail;
            // self._jin.push(data[0]);
            // self._jin.push(data[1]);
            var gameChild = self.node.getChildByName("game");
            gameChild.getChildByName("jininfo").getChildByName("jp1").active = false;
            gameChild.getChildByName("jininfo").getChildByName("jp2").active = false;
            for(var i = 0;i < data.length;i++){
                self._jin.push(data[i]);
                var jp = "jp" + (i+1);
                var jinSprite = gameChild.getChildByName("jininfo").getChildByName(jp).getComponent(cc.Sprite);
                gameChild.getChildByName("jininfo").getChildByName(jp).active = true;
                jinSprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",self._jin[i]);
            }
            if(!cc.vv.gameNetMgr.conf.fenghaozi&&cc.vv.gameNetMgr.conf.type=="kdd"){
                this.node.getChildByName("game").getChildByName("jininfo").active = false;
            }
            ...
            // var jinSprite1 = gameChild.getChildByName("jininfo").getChildByName("jp1").getComponent(cc.Sprite);
            // var jinSprite2 = gameChild.getChildByName("jininfo").getChildByName("jp2").getComponent(cc.Sprite);
            // jinSprite1.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",self._jin[0]);
            // jinSprite2.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",self._jin[1]);
            // console.log("Has Showed jin !!!!");
        });
        // this.node.on('game_sync',function(data){
        //     self._jin = [];
        //     data = data.detail;
        //     for(var i = 0;i < data.jin.length;i++){
        //         self._jin.push(data.jin[i]);
        //     }
        //     var gameChild = self.node.getChildByName("game");
        //     var jinSprite1 = gameChild.getChildByName("jininfo").getChildByName("jp1").getComponent(cc.Sprite);
        //     var jinSprite2 = gameChild.getChildByName("jininfo").getChildByName("jp2").getComponent(cc.Sprite);
        ...
        this.node.on('game_mopai',function(data){
            ...
            if(localIndex == 0){
                ...
                console.log('ready to change mopai');
                sprite.node.getChildByName("isjin").active = false;
                for(var i = 0;i < self._jin.length;i++){
                    if(pai == self._jin[i]){
                        sprite.node.getChildByName("isjin").active = true;
                    }
                }
        ...
        this.node.on('ting',function(data){
            self.showTingPai(data.detail);
        });
        ...
        this.node.on('game_ready_finish',function(data){
            self.initMahjongs();
        });
        ...
        this.node.on("guohu_notify",function(data){

            self.showQiHu(data.detail);
    ...
    addTingOption:function(duoyu){
        ...
        var tingops = this._options.getChildByName("tingops");
        
        var lackingNum = (seatData.pengs.length + seatData.angangs.length + seatData.diangangs.length+ seatData.wangangs.length )*3;
        for(var i = 0; i < holds.length; ++i){
            for(var j = 0;j < duoyu.length;j++){
                if(this._myMJArr[i+lackingNum].node.mjId == duoyu[j]){
                    tingops.children[i+lackingNum].mjId = this._myMJArr[i+lackingNum].node.mjId;
                    tingops.children[i+lackingNum].active = true;
                    break;
                }
            }
        }
    ...
    hideOptions:function(data){
        ...
        var tingops = this._options.getChildByName('tingops');
        for(var i = 0;i < tingops.childrenCount;i++){
            tingops.children[i].mjId = -1;
            tingops.children[i].active = false;
        }
    },

    showTingPai:function(data){
        var tingpai = this._tingpaiTips[data];
        tingpai.active = true;
    },
    showQiHu:function(data){
        var qihu = this._qihus[data];
        qihu.active = true;
        cc.vv.audioMgr.playSFX("qihu.mp3");
    ...
    showAction:function(data){
        ...
        if(data && (data.hu || data.gang || data.peng || data.ting)){
            ...
            if(data.ting){
                // for(var i = 0; i < data.duoyu.length;i++){
                //     var tp = data.duoyu[i];
                //     this.addOption(tp);
                // }
                this.addTingOption(data.duoyu);
            }
        }
    },
    ...
    onGameBeign:function(){
        ...
        for(var i = 0; i < cc.vv.gameNetMgr.seats.length; ++i){
            ...
            var tingpai = this._tingpaiTips[localIndex];
            var qihu = this._qihus[localIndex];
            qihu.active = false;

            console.log("啦啦啦啦啦");
            tingpai.active = false;
        ...
        console.log("QiHuSYNC!!!!!!!!"+cc.vv.gameNetMgr.isQiHu.length+"  "+cc.vv.gameNetMgr.isQiHu);
        for(var i = 0;i<cc.vv.gameNetMgr.isQiHu.length;i++){

            
           var index =  this.getLocalIndex( cc.vv.gameNetMgr.isQiHu[i]) ;
           console.log(index);
           var qihu = this._qihus[index];
            qihu.active = true;


        }
        for(var i = 0;i<cc.vv.gameNetMgr.isTing.length;i++){

            var index =  this.getLocalIndex( cc.vv.gameNetMgr.isTing[i]) ;
             var tingpai = this._tingpaiTips[index];
             tingpai.active = true;
    ...
    onMJClicked:function(event){
       if(this._chupaiArr) 
        ...
        console.log(this._chupaiArr.length);
        //如果不是自己的轮子，则忽略
        if(cc.vv.gameNetMgr.turn != cc.vv.gameNetMgr.seatIndex){
    ...
    onPKClicked:function(event){
        ...
        for(var i = 0; i < this._myPKArr.length; ++i){
            if(event.target == this._myPKArr[i].node){
                //如果是再次点击，则拿出出牌列表
                    for(var i = 0;i<_chupaiArr.length;i++){

                        if(event.target.pkid == this._chupaiArr[i]){

                            event.target.y = 0;
                            _chupaiArr.splice(i,1);
                            return;
                ...
                event.target.y = 15;
                this._chupaiArr.push(event.target.pkid);
                return;
            }
        }
    },
//..........................................................
    onShootPK:function(){
        ...
        if(this._chupaiArr.length<=0){

            return;
        }

        cc.vv.net.send('pk_chupai',_chupaiArr);

        this._chupaiArr = [];

    },
    onBtnQiangDiZhuClicked:function(){

    },
    onBtnBuQiangClicked:function(){

    },
    onBtnTiShiClicked:function(){

    },
    onBtnBuChuClicked:function(){
    ...
    initOtherMahjongs:function(seatData){
        ...
        var num = seatData.pengs.length + seatData.angangs.length +seatData.wangangs.length + seatData.diangangs.length ;
        num *= 3;
        for(var i = 0; i < num; ++i){
    ...
    initMahjongs:function(){
        ...
        //初始化手牌
        var lackingNum = (seatData.pengs.length + seatData.angangs.length + seatData.wangangs.length + seatData.diangangs.length )*3;
        for(var i = 0; i < holds.length; ++i){
            var mjid = holds[i];
            ...
            sprite.node.getChildByName("isjin").active = false;
            for(var ii = 0;ii < this._jin.length;ii++){
                if(mjid == this._jin[ii]){
                    sprite.node.getChildByName("isjin").active = true;
                }
            }
        }
    ...
    initPokes:function(){
        ...
        for(var i = 0; i < holds.length; ++i){
            var pkid = holds[i];
            var sprite = this._myPKArr[i];
            sprite.node.pkid = pkid;
            sprite.node.y = 0;
            this.setSpriteFrameByMJID("M_",sprite,mjid);         
        }
    ...
    onOptionClicked:function(event){
        ...
        if(event.target.name == "btnPeng"){
            ...
            cc.vv.net.send("ting",event.target.pai);
        }else if(event.target.name == "mark_ting"){
            cc.vv.net.send("ting",event.target.mjId);
        }
    },
