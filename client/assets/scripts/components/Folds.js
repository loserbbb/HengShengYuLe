...
cc.Class({
    ...
    onLoad: function () {
        ...
        this._jin = [];
        for(var i = 0;i < cc.vv.gameNetMgr.jin.length;i++){
    ...
    initEventHandler:function(){
        ...
        this.node.on('game_getjin',function(data){
            var k;
            self._jin = [];
            for(k = 0;k < data.detail.length;k++){
                self._jin.push(data.detail[k]);
            }
            console.log('jin has push');
        });
    ...
    initFolds:function(seatData){
        ...
        for(var i = 0; i < foldsSprites.length; ++i){
            ...
            for(var k = 0;k < this._jin.length;k++){
                if(this._jin[k] == folds[i]){
    ...
    setSpriteFrameByMJID:function(pre,sprite,mjid){
        ...
        console.log("pre = "+pre+"     mjid = " + mjid);
        sprite.node.active = true;
        // if(mjid<0)
        // if (pre == "M_" || pre == "B_") {
            
        //     sprite.node.scaleX = 1.48;
        //     sprite.node.scaleY = 1.48;
