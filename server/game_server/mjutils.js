function checkTingPai(seatData,begin,end,game){
	for(var i = begin; i < end; ++i){
		...
		for(var j=0;j<game.jin.length;j++){
			if(game.jin[j] == i){
				haveJin = true;
			}
		}
...
function matchSingle(seatData,selected){
	...
	if(v < 2){
		...
		var thereJinCount = seatData.jinCount;
		for(var i = 0; i < 3; ++i){
			var t = selected - 2 + i;
			var cc = seatData.countMap[t];
			if(cc == null){
				seatData.countMap[t] = 0;
				cc = 0;
			}
			if(cc == 0){
				if(thereJinCount == 0){
	...
	//匹配成功，扣除相应数值
	if(matched){
		var c1 = seatData.countMap[selected - 2];
		var c2 = seatData.countMap[selected - 1];
		var c3 = seatData.countMap[selected];
		if(c1 == 0){
	...
	//匹配成功，扣除相应数值
	if(matched){
		var c1 = seatData.countMap[selected - 1];
		var c2 = seatData.countMap[selected];
		var c3 = seatData.countMap[selected + 1];
		if(c1 == 0){
			seatData.countMap[selected - 1] ++;
			seatData.jinCount --;
		}
	...
	//匹配成功，扣除相应数值
	if(matched){
		var c1 = seatData.countMap[selected];
		var c2 = seatData.countMap[selected + 1];
		var c3 = seatData.countMap[selected + 2];
		if(c1 == 0){
			seatData.countMap[selected] ++;
			seatData.jinCount --;
		}
...
function checkSingle(seatData){
	...
	}else if(c + seatData.jinCount >= 3){
		//直接作为一坎
		seatData.countMap[selected] = 0;
		seatData.jinCount -= 3 - c;
		debugRecord(selected);
		debugRecord(selected);
		debugRecord(selected);
		var ret = checkSingle(seatData);
...
function checkCanHu(seatData){
	for(var k in seatData.countMap){
		...
		if(c + seatData.jinCount < 2){
			continue;
		}
		if(c < 2){
			seatData.jinCount -= 2 - c;
			seatData.countMap[k] += 2 - c;
		}
		//如果当前牌大于等于２，则将它选为将牌
